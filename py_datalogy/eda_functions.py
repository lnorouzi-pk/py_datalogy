...
def analyze_numeric(df, visual=True, save_pic=True, path='') -> None:
    '''
    ...
    :param visual: if the user wants to plot the results
    ...
    '''
    if visual:
        print("******************Plotting for numeric variables*********************")
...
def analyze_cat(df, save_pic=True, visual=True, path='') -> None:
    ...
    if visual:
        print("******************Plotting for non-numeric variables*********************")
...
def analyze_numeric_correlations(df, visual=True, save_pic=True, path='') -> None:
    '''
    ...
    print('######visual in corr :',visual)
    if len(list(df.columns))>1:

        corr_data = df.corr()

        if visual:
            print("******************Plotting correlation for numeric variables*********************")
            ...
            fig_name = path+'EDA_numeric_cor_plot' + ''.join(i[0].upper() for i in self.data.columns) + '.png'
            if save_pic: fig.savefig(fig_name,  dpi=100)

            plt.show()
            plt.close(fig)
        else:
            print('****************No correlation will be plotted******************')
    else:
        if visual:
            fig, ax = plt.subplots(figsize=(8, 8), constrained_layout=True)
            plt.text(0.5, 0.5, 'No missing value in any variables \ncoincide with a numeric variable for\n%s' % (
                ' '.join(i for i in df.columns.to_list())), ha='center')
            ax.set_title('Correlation between variables', fontsize='large')
            ax.set_xlabel('Varibales')
            ax.set_ylabel('Varibales')
            fig_name = path+'EDA_numeric_cor_plot' + ''.join(i[0].upper() for i in df.columns) + '.png'
            if save_pic: fig.savefig(fig_name, dpi=100)
            plt.show()
            plt.close(fig)
        else:
            print('****************No correlation will be plotted, no correlation******************')

    pass
#
# def analyze_pair(df:pd.DataFrame, x1: str, x2:str, visual= True, save_pic=True, path='') -> None:
#     '''
...
#
#     if visual:
#         print("******************Plotting pair for two variables*********************")
#         fig, ax = plt.subplots(figsize=(17, 8), constrained_layout=True)
#
